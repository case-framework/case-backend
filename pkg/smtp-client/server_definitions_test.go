...
func TestReadServerConfigsFromFile(t *testing.T) {
	...
	t.Run("with wrong filename", func(t *testing.T) {
		err := servers.ReadFromFile("../../test/configs/smpt-server/nothere.yaml")
		if err == nil {
	...
	t.Run("with wrong content", func(t *testing.T) {
		...
		err := servers.ReadFromFile("../../test/configs/smpt-server/smtp-servers-wrong.yaml")
		if err == nil {
			t.Error("error expected")
		}
	})
	...
	t.Run("with valid content", func(t *testing.T) {
		err := servers.ReadFromFile("../../test/configs/smtp-server/smtp-servers.yaml")
		if err != nil {
